...
struct PodcastIndexDiscoveryService: PodcastDiscoveryServiceProtocol {
    ...
    func fetchTechnologyPodcasts(limit: Int, offset: Int) async throws -> [DiscoveredPodcast] {
        ...
        guard let request = buildRequest(
            endpoint: "https://api.podcastindex.org/api/1.0/podcasts/trending",
            queryItems: [URLQueryItem(name: "max", value: String(limit))]
        ) else { return [] }

        return try await fetchPodcasts(request: request)
    }
    ...
    private func buildRequest(endpoint: String, queryItems: [URLQueryItem]) -> URLRequest? {
    ...
    private func fetchPodcasts(request: URLRequest) async throws -> [DiscoveredPodcast] {
        ...
        return feeds.compactMap(mapFeed)
    }

    private func mapFeed(_ feed: PodcastIndexFeed) -> DiscoveredPodcast? {
